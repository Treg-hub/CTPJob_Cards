...
- **Startup resilience + navigation/layout polish (2026-07-03, branch `fix/startup-resilience`)**:
  ...
  - **Navigation/layout**: one `PageTransitionsTheme` (Cupertino slide) on both themes in `main.dart` — lighter than Android's zoom default (kills back-nav stutter) + edge-swipe-back. Desktop Home grid constrained to 1200px centred column + 1.25 tile aspect (`_maxContentWidth`/`_gridChildAspectRatio`) so wide screens stop ballooning tiles / hiding Recent Job Cards; icon 60→44 desktop; phones/tablets unchanged.
