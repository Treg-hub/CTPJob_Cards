...
- **Startup / Update surfaces (2026-07-03; in-app install 2026-07-08)**: Three layers, Job Cards mobile only. (1) Kill-switch settings/app.minSupportedBuild pre-runApp -> UpdateRequiredScreen / shared UpdateAvailableScreen + updateDownloadUrl (fails open). (2) UpdateService Remote Config (latest_version/latest_build/download_url/force_update/release_notes, optional apk_sha256); Home mount + resume (4h cooldown); force = non-dismissible full-screen. In-app APK: ApkInstallService + MethodChannel ctp/apk_install + FileProvider -> system installer; browser fallback; exit kiosk before install. (3) WhatsNewService top docs/CHANGELOG.md entry once per build; stamp after sheet dismiss; onboarding stamps fresh installs. Settings shows version + Check for Update. Keep RC download_url and kill-switch URL in sync.
