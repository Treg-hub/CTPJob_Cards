...
_Last updated: 2026-07-12 (Feedback photos — `photoUrls` on submit + thread replies; Storage `feedback/{id}/photos/*`. Prior: 2026-07-07 Loading regression audit — View Jobs `getViewJobCardsBundleWithMeta`, Daily Review `getActiveAndRecentlyClosedJobCardsWithMeta`, Notification Inbox + on-site stock list use resilient streams + `decideListLoadState`; shared `getJobCardsByStatusWithMeta` + `mergedIsFromCache`/`allStreamSidesReady`. Prior: My Work merge fix, Waste home active loads, stock qty editor, fleet daily check `SafeBottomBar`.) 2026-07-07 (Firestore cost remediations — `counters/job_cards_open` + jobcards CFs `onJobCardOpenCounts`/`reconcileOpenJobCardCounts`; Pulse board KPIs via 1-doc counter listener; `/jobs` hybrid live-open + paginated closed; Home query bounds: active jobs `limit(150)` + open-status `whereIn`; inbox unread `limit(50)`; ink replay `ink_replay` structured logging in wastetrack-overtime. See monorepo `docs/Firestore_Cost_Discipline.md`.) 2026-07-03 (Feedback loop closed — home FAB now opens **My Feedback** (`my_feedback_screen.dart`, all roles incl. Guard-Shell): submit + follow own items with worker-friendly statuses (Received/Planned/Done/Declined) and a public two-way thread (`feedback_thread_screen.dart`, `feedback/{id}/feedback_comments` subcol — submitter + admins only, rules-enforced via clockNum claim); admin board gains "Reply to submitter"; CFs `onFeedbackStatusChanged`/`onFeedbackCommentCreated` (this repo's jobcards codebase) notify the other side, push or inbox-park with `feedbackId` deep link; feedback update/delete now admin-claim-only in rules). 2026-07-02 (Waste offline resilience — persistent `waste_media_queue/` media, single-owner Hive queue, media_lost audit surfacing, timestamp restoration + photo_count recompute on replay, IBC pool ops online-only guard, 14-day home window). 2026-07-02 (Client off-site presence gating — Ink/Waste/Security hidden off-site; Fleet reporters on-site only, mechanics full access; My Work retained; Create blocked unless isAdmin; see `lib/utils/presence_gating.dart`). 2026-06-28 (iPhone web inbox_only delivery — clientPlatform/clientDevice on employees; CF prefersInboxDelivery parks all job alerts; Android push unchanged)._
...
- Pulse surface: monorepo `web/ctp-pulse/src/features/board/components/JobCardsModule.tsx` + `useJobCardMetrics.ts` + `useOpenJobCards.ts` (reads `counters/job_cards_open`, 1-doc listener) + hybrid `/jobs` page (live open + paginated closed). Cost discipline: `../../../docs/Firestore_Cost_Discipline.md`
...
| **My Timesheet worker** | `clockNo` ∈ `work_report_settings.enabled_clock_nos` (+ module enabled) | Home tile → monthly job hours + additional work + PDF/CSV for Accounts. Inclusion via CF `refreshWorkReportJobLines` (client fallback). Soft-lock after PDF; post-PDF edits audited. Rules enforce enrolment on write; managers read on Pulse Team Timesheets. Config: Pulse Settings → My Timesheet (incl. factory pay-period mode). |
...
| `feedback_admin_screen.dart` | admin | **User Feedback triage board** — Home Quick Actions **Feedback** tile. Status `New → Planned → Implemented → Declined` + PRIVATE notes; photo thumbs; thread via `feedback_thread_screen.dart`. |
...
**User Feedback loop (`feedback` collection + `feedback_comments` subcol, closed 2026-07-03; photos 2026-07-12)**: the Home-screen FAB opens **My Feedback** (`my_feedback_screen.dart`, all roles) — submit (`feedback`/`userName`/`clockNo`/`timestamp`/`photoUrls?`, persona-attributed to the real employee; up to 3 photos via `FeedbackService` → Storage `feedback/{id}/photos/*`) and follow own items with worker wording (Received/Planned/Done/Declined) + reply counts. Tapping an item (or a parked `feedback_status`/`feedback_comment` inbox notification, deep-linked by `feedbackId`) opens the shared **thread** (`feedback_thread_screen.dart`): original message + photos + status + public comments (replies may also carry `photoUrls`); only the original submitter and admins can post — enforced in **rules** (parent `clockNo == clockNum` claim, self-attributed `byClockNo`, non-admins can't set `byIsAdmin`), not just UI. The admin board writes triage fields — `status`, `statusUpdatedAt`, `statusUpdatedByClockNo`, `adminNotes*` (PRIVATE, never in the thread) — now gated `update, delete: if isAdmin()` in rules; `lastCommentAt`/`commentCount` are CF-maintained. CFs `onFeedbackStatusChanged` + `onFeedbackCommentCreated` (this repo's `functions/index.js`) notify the submitter on status/replies and all admins on submitter replies (push on-site, inbox park off-site). Status filtering and My-Feedback sorting are client-side, so no composite index is needed. Shared model: `lib/models/feedback_item.dart`.
...
- **Number Assignment (counters)**: jobCards / overtime / waste / fleet / ink counters. Client read, CF/AdminSDK write only (Wave B). Global sequential never-reset for waste/fleet/ink. **`counters/job_cards_open`** (2026-07-07): Pulse board active/critical KPIs — maintained by `onJobCardOpenCounts` + admin `reconcileOpenJobCardCounts` (jobcards CF bundle); mobile Home does **not** listen to this doc (uses bounded `getActiveJobCardsWithMeta` instead).
- **Firestore cost discipline (2026-07-07)**: Bounded Home listeners in `firestore_service.dart` — active jobs `whereIn` open statuses + `limit(150)` (incl. legacy display-name variants); my-work assigned/created queries `limit(80)` each (composite indexes on `assignedClockNos`+`status`, `operatorClockNo`+`status`); inbox unread `limit(50)`. Pulse: counter doc for KPIs; `/jobs` live open `limit(150)` + closed history paginated `getDocs`. Board attention: shared ref-counted waste/OT listeners + `useMemo` (no `getDocs` storm). Full checklist: monorepo `docs/Firestore_Cost_Discipline.md`.
- **Pulse Job Cards view**: External read-only KPIs (see cross-links above). Not full CRUD. "Job Cards & Machine Health" branding in Pulse. Active/critical counts from `counters/job_cards_open` (not an unbounded open-job_cards listener).
- **Startup / Update surfaces (2026-07-09 targeted channels)**: Kill-switch (`minSupportedBuild`); `UpdateService` + `updateChannels` (testers → ink/depts → default); soft = Home banner (24h snooze); force = full-screen; 24h network check; ApkInstallService; Admin multi-select depts/people; legacy publish fields = default only. Guide: `docs/admin_app_update_guide.md`. WhatsNew multi-build rollup; persona onboarding. See JobCardsCoreModule.md.
...
- **Navigation & wide-screen layout (2026-07-03, grid refined 2026-07-04)**: `main.dart` sets one `PageTransitionsTheme` (Cupertino slide) for all routes on both themes — lighter to paint than the Android zoom default (removes back-nav stutter) and adds edge-swipe-back. Home Quick Actions: colour-grouped flat tiles (orange job cards, **cyan** ink `kInkModule` `#06B6D4`, slate fleet, gold Daily Review), 10px radius, fixed `_gridTileHeight` + `mainAxisExtent`; phones 3 columns, tablet/desktop `SliverGridDelegateWithMaxCrossAxisExtent` (≤210px) — supersedes the brief 1200px centred-column attempt.
- **JobCardTile + Home lists (2026-07-04)**: shared `widgets/job_card_tile.dart` — flat card, priority border + left accent, compact description, activity inset block, orange job-number badge; used on manager **Recent Job Cards** (managers + General dept only), View Jobs, My Work, History, Daily Review. `ink_daily_readings_banner.dart` uses the same cyan Ink tint when meter readings are incomplete.
