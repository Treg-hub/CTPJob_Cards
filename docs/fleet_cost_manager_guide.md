...
Mechanics log **what they did** on Hyster machines (forks or grab attachments). You record **what it cost** — parts, invoices, labour, and other spend. Mechanics never see amounts; only you and admins do.
...
| **Reports** | Totals, spend per Hyster, and export for accounts |
...
You can also filter by **Hyster** or **Job type**.

### 2. General cost (not tied to a job)

Use for spend on a Hyster that is **not** linked to one mechanic job — e.g. annual contract, delivery fee, stock not tied to a specific repair.

1. On the **Costs** tab, tap **General cost** (bottom-right)
2. Pick the Hyster, category, description, and amount
...
|----------|-------------|
| **Parts** | Parts or materials bought for a Hyster |
...
|-------|-----------|--------|
| Which Hyster? | Yes | The machine this spend applies to |
...
3. See:
   ...
   - **Spend per Hyster** (bar chart)
   - Full list of every cost line
4. Tap **Export CSV** to share a spreadsheet (e.g. email to accounts)

The CSV includes date, Hyster, job number (if linked), category, description, amount, invoice, supplier, and who entered it.
...
- **General costs** without a job link still appear in Reports under the Hyster name.
