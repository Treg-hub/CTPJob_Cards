...
/// screen writes an `adjustment` transaction for the delta (counted − current),
/// moved at the current WAC (adjustments never change WAC).
class InkAdjustmentScreen extends ConsumerStatefulWidget {
  const InkAdjustmentScreen({super.key});

  @override
  ConsumerState<InkAdjustmentScreen> createState() => _State();
}

class _State extends ConsumerState<InkAdjustmentScreen> {
  ...
  Future<void> _submit(InkStockItem item) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Adjustment recorded (${delta > 0 ? '+' : ''}'
              '${_qty.format(delta)} ${item.unit}).')));
    ...
    return Scaffold(
      ...
      body: itemsAsync.when(
        ...
        data: (items) {
          ...
          final counted = double.tryParse(_countedCtrl.text.trim());
          final delta =
              (selected != null && counted != null) ? counted - selected.currentBalance : null;
          return Form(
            key: _formKey,
            child: ListView(
              ...
              children: [
                ...
                if (selected != null) ...[
                  ...
                  Text(
                    ...
                    '${_qty.format(selected.currentBalance)} ${selected.unit}',
                ...
                TextFormField(
                  ...
                  validator: (v) {
                    ...
                    if (d == null || d < 0) return 'Enter the counted quantity';
                    return null;
                  },
                ),
                if (delta != null) ...[
                  const SizedBox(height: 8),
                  Text(
                    'Adjustment: ${delta > 0 ? '+' : ''}${_qty.format(delta)} '
                    '${selected!.unit}',
