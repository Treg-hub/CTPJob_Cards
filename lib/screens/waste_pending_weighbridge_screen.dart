...
class WastePendingWeighbridgeScreen extends ConsumerStatefulWidget {
  ...
  final bool embedded;
  const WastePendingWeighbridgeScreen({super.key, this.embedded = false});
...
class _WastePendingWeighbridgeScreenState extends ConsumerState<WastePendingWeighbridgeScreen> {
  ...
  Widget build(BuildContext context) {
    if (!_canAccess) {
      if (widget.embedded) {
        return const Center(child: Text('Access denied. Waste Admin or Security Manager only.'));
      }
    ...
    if (!_effectiveWasteEnabled) {
      ...
      if (widget.embedded) return disabledBody;
      return Scaffold(
