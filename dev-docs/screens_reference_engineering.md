...
| Tile | Screen |
|------|--------|
| Receive Local | `InkSelectLocalOrderScreen` → `InkReceiveRawMaterialScreen` (multi-line confirm; ad-hoc escape). List also shows greyed **Received this period** fulfilled local POs (open count window; read-only). |
| Receive Ink (IBC) | `InkSelectIbcShipmentScreen` → `InkReceiveIbcScreen`. List also shows greyed **Received this period** shipments (`received` / `awaiting_grn` / `costed`; open count window; read-only). |
...
Scans ink IBCs (GS1-128 barcode labels). List-first via `InkSelectIbcShipmentScreen` (outstanding `awaiting_receipt` / `receiving`, plus greyed **Received this period** section for shipments received after the latest month-end count). On submit, cost-pending purchases + IBC register (CF path).

### Receive Local

`lib/screens/ink_select_local_order_screen.dart` → `ink_receive_raw_material_screen.dart` — **Roles:** Ink operator, Ink manager, Admin

Outstanding local POs (`sent` / `partially_fulfilled`) plus greyed **Received this period** fulfilled local POs (same open count window). Received rows are visual reference only (snackbar; no re-receive).
---
